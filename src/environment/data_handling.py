...
def compute_agent_activity_durations(
    ...
    Args:
        ...
        use_work_hours: If True, compute durations using working hours only
            (Mon-Fri 08:00-20:00), filtering out nights/weekends. This gives
            realistic processing times instead of inflated calendar durations.
    ...
    for _, row in data.iterrows():
        ...
        if use_work_hours:
            duration = working_seconds_between(
...
def compute_activity_duration_distribution_per_agent(
    ...
    Args:
        ...
        percentile_cap: If set, clip durations above this percentile before
            fitting. Defaults to None (disabled) — use_work_hours handles
            overnight/weekend outliers instead.
        use_work_hours: If True, compute durations using working hours only
            (Mon-Fri 08:00-20:00). This naturally removes overnight/weekend
            inflation, making the P95 cap unnecessary.
    ...
    """
    activity_durations_dict = compute_agent_activity_durations(data, use_work_hours=use_work_hours)
    ...
    for agent, val in activity_durations_dict.items():
        for act, duration_list in val.items():
            if len(duration_list) > 0:
                ...
                # Apply percentile cap to remove outliers (e.g. overnight/weekend tasks)
                if percentile_cap is not None and len(duration_list) > 1:
                    cap_value = float(np.percentile(duration_list, percentile_cap))
                    n_before = len(duration_list)
                    duration_list = [d for d in duration_list if d <= cap_value]
                    n_removed = n_before - len(duration_list)
                    if n_removed > 0:
                        print(
                            f"P{percentile_cap} cap for {agent}/{act}: "
                            f"removed {n_removed}/{n_before} outliers "
                            f"(cap={cap_value:.1f}s)"
                        )

                # Calculate statistics (on capped data)
...
def compute_global_activity_medians(
    data: pd.DataFrame,
    percentile_cap: Optional[float] = None,
    use_work_hours: bool = True,
) -> Dict[str, float]:
    ...
    Args:
        data: Event log in pandas format
        percentile_cap: If set, clip durations above this percentile before
            computing medians. Defaults to None (disabled).
        use_work_hours: If True, compute durations using working hours only
            (Mon-Fri 08:00-20:00). Consistent with per-agent fitting.
    ...
    for activity in activities:
        ...
        # Apply percentile cap to remove outliers (optional, disabled by default)
        if percentile_cap is not None and len(durations) > 1:
            cap_value = float(np.percentile(durations, percentile_cap))
            durations = [d for d in durations if d <= cap_value]
